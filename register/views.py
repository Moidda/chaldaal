...
def sign_up_verification(request):
    if request.method == 'POST':
        ...
        # check if email is alreaady in use by another customer
        for row in result:
...
# retrieve the input user gave from the webpage
# check if such a user_email exists in database
# and cross check password
...
def log_in_verification(request):
    if request.method == 'POST':
        ...
        result = cursor.fetchall()
        customer_pass_in_db = result[0][0]
